...
<?php 
	$page_title = ig_custom_get_page_title();
...
<div id="content">
	<section id="blog" class="<?php echo $sectionpadding; ?> <?php echo $options['blog_type']; ?>">
		<div class="container">
            ...
            if($blog_type == 'standard-blog'){
				...
				if (in_array('search-no-results',$classes)) {
					echo '<div id="post-area" class="no-isotope">';
				} else {
		...
		<?php if($blog_type == 'masonry-blog' || $blog_type == 'center-blog'){ ?>
		<div class="col-md-12">
			<div class="no-results"> 
                <h2 class="entry-title"><?php _e('Your search did not match any entries!', INFINITE_LANGUAGE) ?></h2>
                <p><?php _e('Sorry, but you are looking for something that isn\'t here.', INFINITE_LANGUAGE) ?></p>
			</div>
		</div>
		<?php } else { ?>
		<div class="no-results"> 
