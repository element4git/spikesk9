...
if ( !class_exists( 'mega_main_init' ) ) {
	class mega_main_init {
		...
		public function constant( $mm_config = array() ) {
			...
			$theme_includes = get_template_directory_uri() . '/includes';
			...
			$this->constant[ 'INFINITE_LANGUAGE' ] = $this->constant[ 'MM_WARE_SLUG' ] . '_admin';
			...
			// URL's			
			$this->constant[ 'MM_WARE_URL' ] = $theme_includes;
			$this->constant[ 'MM_WARE_CSS_URL' ] = $this->constant[ 'MM_WARE_URL' ] . '/css/menu';
			$this->constant[ 'MM_WARE_JS_URL' ] = $this->constant[ 'MM_WARE_URL' ] . '/js/menu';
			$this->constant[ 'MM_WARE_FONTS_URL' ] = $this->constant[ 'MM_WARE_URL' ] . '/css/fonts';
			$this->constant[ 'MM_WARE_IMG_URL' ] = $this->constant[ 'MM_WARE_URL' ] . '/img/menu';
		}
		...
		public function options_page() {
			$out = '';
			$submit_button = mm_common::ntab(7) . '<input type="submit" class="button-primary pull-right" value="' . __( 'Save All Changes', $this->constant[ 'INFINITE_LANGUAGE' ] ) . '" />';
			$theme_meta = mm_common::ntab(7) . '<div>' . mm_common::ntab(8) . '<span class="theme_name">' . __(  $this->constant[ 'MM_WARE_NAME' ] , $this->constant[ 'INFINITE_LANGUAGE' ] ) . '</span>' . ' <small>v' . $this->constant[ 'MM_WARE_VERSION' ] . mm_common::ntab(7) . '</small></div>';
		...
		public function load_all_src ( $args ) {
			...
			if ( is_admin() ) {
				foreach ( $ware_src_array[ 'backend' ][ 'css' ] as $key => $value ) {
					...
					// Register 
					wp_register_style( 'style-fonts', $this->constant[ 'MM_WARE_FONTS_URL' ] .  '/style-fonts.css');
					wp_register_style( 'font-awesome', $this->constant[ 'MM_WARE_CSS_URL' ] .  '/external/font-awesome.css');
					wp_register_style( 'bootstrap', $this->constant[ 'MM_WARE_CSS_URL' ] .  '/external/bootstrap.css');
					wp_register_style( 'colorpicker', $this->constant[ 'MM_WARE_CSS_URL' ] .  '/external/colorpicker.css');
					wp_register_style( 'common', $this->constant[ 'MM_WARE_CSS_URL' ] .  '/backend/common.css');
					wp_register_style( 'icons_modal', $this->constant[ 'MM_WARE_CSS_URL' ] .  '/backend/icons_modal.css');
					...
					// Enqueue
					 ...
					 wp_enqueue_style('icons_modal');
				}
				foreach ( $ware_src_array[ 'backend' ][ 'js' ] as $key => $value ) {
					if ( $value != '' ) {
						// Register
						wp_register_script('bootstrap-js', $this->constant[ 'MM_WARE_JS_URL' ] . '/external/bootstrap.js', 'jquery', '3.9.1', TRUE);
						wp_register_script('bootstrap-colorpicker', $this->constant[ 'MM_WARE_JS_URL' ] . '/external/colorpicker.js', 'jquery', '3.9.1', TRUE);
						wp_register_script('option-generator', $this->constant[ 'MM_WARE_JS_URL' ] . '/backend/option_generator.js', 'jquery', '3.9.1' ,TRUE);
					}
					// Enqueue
					wp_enqueue_script('bootstrap-js');
					wp_enqueue_script('bootstrap-colorpicker');
					wp_enqueue_script('option-generator');
				}
			} else {
				foreach ( $ware_src_array[ 'frontend' ][ 'js' ] as $key => $value ) {
					if ( $value != '' ) {
						// Register
						wp_register_script('menu-functions', $this->constant[ 'MM_WARE_JS_URL' ] . '/frontend/menu_functions.js', 'jquery', '3.9.1', TRUE);
					}
					// Enqueue
