...
//	header("Content-type: text/css", true);
	if ( $_GET[ $mega_main_menu->constant[ 'MM_WARE_PREFIX' ] . '_page' ] == 'icons_list' ) {
	...
	<script type="text/javascript">
		...
		echo mm_common::ntab(6) . '<input type="text" class="all_icons_search_input '. $modal_id .'" placeholder="'.__( 'Search icon', $mega_main_menu->constant[ 'INFINITE_LANGUAGE'] ).'">';
		echo mm_common::ntab(6) . '<span class="ok_button btn-primary" onclick="mm_icon_selector(\'' . $input_name . '\', \'' . ( isset( $_GET['modal_id'] ) ? $_GET['modal_id'] : '') . '\' );">'.__( 'OK', $mega_main_menu->constant[ 'INFINITE_LANGUAGE'] ).'</span>';
		echo mm_common::ntab(5) . '</div><!-- class="all_icons_control_panel" -->';
