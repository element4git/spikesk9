...
class IG_Widget_Dribbble extends WP_Widget {
	
	function IG_Widget_Dribbble() {

		$widget_ops = array(
			'classname' => 'ig_widget_dribbble',
			'description' => __('Use this widget to display your Dribbble shots.', INFINITE_LANGUAGE)
		);
		$control_ops = array('id_base' => 'ig_widget_dribbble');
		$this->WP_Widget('ig_widget_dribbble', __('Custom Dribbble Widget', INFINITE_LANGUAGE), $widget_ops, $control_ops);
					...
					$.getJSON("http://api.dribbble.com/players/<?php echo $dribbble_ID; ?>/shots?callback=?", function(data) {
						$.each(data.shots, function(index, shot) {
							if(index < <?php echo $limit; ?>) {
								$(".ig_dribbble_widget").append("<li class='item-" + $i + "'><a class='dribbble-item" + "' href='" + shot.image_url + "'><img src='" + shot.image_teaser_url + "'></a></li>");
								$i++;
							}
						});
					});
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', INFINITE_LANGUAGE) ?></label>
			<input class="widefat" type="text" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>">
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('dribbble_ID'); ?>"><?php _e('Dribbble Username:', INFINITE_LANGUAGE) ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('limit'); ?>"><?php _e('Number of Shots:', INFINITE_LANGUAGE) ?></label>
...
add_action('widgets_init', 'ig_dribbble_widget');

function ig_dribbble_widget() {
	register_widget('IG_Widget_Dribbble');
}